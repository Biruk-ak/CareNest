...
import type { InventoryRecord } from '@/modules/inventory/types/inventory';
import { useInventoryMutations } from '@/modules/inventory/hooks/inventory';

export function InventoryDetailPanel({ id }: { id: string }) {
  const [record, setRecord] = useState<InventoryRecord | null>(null);
  const [error, setError] = useState<string | null>(null);
  const { update, busy } = useInventoryMutations();

  useEffect(() => {
    fetch(`/api/inventory/${id}`)
      .then(async (r) => {
        if (!r.ok) throw new Error(await r.text());
        return r.json() as Promise<InventoryRecord>;
      })
      .then(setRecord)
  ...
  return (
    <article className="cn-panel p-6 space-y-4">
      <header className="space-y-1">
        <p className="text-xs uppercase tracking-[0.2em] text-nest-500">Inventory</p>
