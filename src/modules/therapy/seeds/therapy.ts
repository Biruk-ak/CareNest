...
export function seedTherapyRecords(count = 48): TherapyRecord[] {
  const rows: TherapyRecord[] = [];
  for (let i = 0; i < count; i++) {
    const id = `seed-therapy-${String(i + 1).padStart(4, '0')}` as TherapyId;
    const created = new Date(Date.UTC(2024, i % 12, (i % 27) + 1, 8 + (i % 10))).toISOString();
    rows.push({
      id,
      title: `Therapy case #${i + 1}`,
      description: `Seeded therapy workflow item #${i + 1} for CareNest facility operations.`,
      status: STATUSES[i % STATUSES.length],
      priority: PRIORITIES[i % PRIORITIES.length],
      visibility: VIS[i % VIS.length],
      residentId: `resident-${(i % 20) + 1}`,
      caregiverId: `caregiver-${(i % 12) + 1}`,
      tags: ['therapy', `batch-${i % 5}`, i % 2 === 0 ? 'monitored' : 'routine'],
      notes: [`Initial note for therapy #${i + 1}`, `Follow-up checkpoint ${(i % 3) + 1}`],
      metadata: { seedIndex: i, module: 'therapy', weight: i * 1.5 },
      createdAt: created,
      updatedAt: created,
