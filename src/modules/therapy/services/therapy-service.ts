...
import { therapyCreateSchema, therapyFilterSchema, therapyUpdateSchema } from '../validators/therapy';
import { therapyStore } from '../stores/therapy-store';
import { buildTherapyId, nowIso, paginateTherapy } from '../utils/therapy';

/** Application service for Therapy domain operations */
export class TherapyService {
  constructor(private readonly store = therapyStore) {}

  async list(filter: TherapyFilter = {}): Promise<TherapyPage> {
    const parsed = therapyFilterSchema.parse(filter);
    const all = this.store.getAll();
    const filtered = all.filter((item) => matchesTherapyFilter(item, parsed));
    return paginateTherapy(filtered, parsed.page ?? 1, parsed.pageSize ?? 25, parsed.sortBy as keyof TherapyRecord | undefined, parsed.sortDir);
  }

  async getById(id: TherapyId): Promise<TherapyRecord | null> {
    return this.store.getById(id) ?? null;
  }

  async create(input: TherapyCreateInput, actorId: string, facilityId: string): Promise<TherapyRecord> {
    const data = therapyCreateSchema.parse(input);
    const stamp = nowIso();
    const record: TherapyRecord = {
      id: buildTherapyId(),
      title: data.title,
  ...
  async update(input: TherapyUpdateInput, actorId: string): Promise<TherapyRecord> {
    const data = therapyUpdateSchema.parse(input);
    const existing = this.store.getById(data.id as TherapyId);
    if (!existing) throw new Error('Therapy record not found');
    const next: TherapyRecord = {
      ...existing,
      ...data,
      id: existing.id,
      updatedAt: nowIso(),
  ...
  async remove(id: TherapyId): Promise<boolean> {
    return this.store.remove(id);
  }

  async summary(facilityId?: string): Promise<TherapySummary> {
  ...
  async analyzeTherapySignal1(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
  ...
  async analyzeTherapySignal2(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal3(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal4(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal5(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal6(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal7(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal8(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal9(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal10(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal11(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal12(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal13(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal14(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal15(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal16(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal17(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal18(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal19(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal20(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal21(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal22(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal23(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal24(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal25(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal26(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal27(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal28(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTherapySignal29(id: TherapyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
...
function matchesTherapyFilter(item: TherapyRecord, filter: TherapyFilter): boolean {
