...
import { useQualityMetricsList, useQualityMetricsSummary } from '@/modules/quality-metrics/hooks/quality-metrics';
import { formatQualityMetricsHeadline } from '@/modules/quality-metrics/formatters/quality-metrics';

export function QualityMetricsListPanel() {
  const { items, total, loading, error, setFilter, reload } = useQualityMetricsList({ page: 1, pageSize: 20 });
  const { summary } = useQualityMetricsSummary();

  return (
    <section className="cn-panel p-6 space-y-5">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-nest-500">QualityMetrics</p>
          <h2 className="font-display text-2xl text-nest-900">QualityMetrics workspace</h2>
        </div>
      ...
      {loading && <p className="text-sm text-nest-600">Loading quality-metrics records…</p>}
      ...
      <ul className="divide-y divide-nest-100">
        {items.map((item) => (
          <li key={item.id} className="py-3 flex items-start justify-between gap-4">
            <div>
              <Link href={`/app/quality-metrics/${item.id}`} className="font-semibold text-nest-800 hover:text-nest-600">
                {formatQualityMetricsHeadline(item)}
              </Link>
