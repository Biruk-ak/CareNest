...
import { pharmacyCreateSchema, pharmacyFilterSchema, pharmacyUpdateSchema } from '../validators/pharmacy';
import { pharmacyStore } from '../stores/pharmacy-store';
import { buildPharmacyId, nowIso, paginatePharmacy } from '../utils/pharmacy';

/** Application service for Pharmacy domain operations */
export class PharmacyService {
  constructor(private readonly store = pharmacyStore) {}

  async list(filter: PharmacyFilter = {}): Promise<PharmacyPage> {
    const parsed = pharmacyFilterSchema.parse(filter);
    const all = this.store.getAll();
    const filtered = all.filter((item) => matchesPharmacyFilter(item, parsed));
    return paginatePharmacy(filtered, parsed.page ?? 1, parsed.pageSize ?? 25, parsed.sortBy as keyof PharmacyRecord | undefined, parsed.sortDir);
  }

  async getById(id: PharmacyId): Promise<PharmacyRecord | null> {
    return this.store.getById(id) ?? null;
  }

  async create(input: PharmacyCreateInput, actorId: string, facilityId: string): Promise<PharmacyRecord> {
    const data = pharmacyCreateSchema.parse(input);
    const stamp = nowIso();
  ...
  async update(input: PharmacyUpdateInput, actorId: string): Promise<PharmacyRecord> {
    const data = pharmacyUpdateSchema.parse(input);
    const existing = this.store.getById(data.id as PharmacyId);
    if (!existing) throw new Error('Pharmacy record not found');
    const next: PharmacyRecord = {
      ...existing,
      ...data,
      id: existing.id,
      updatedAt: nowIso(),
  ...
  async remove(id: PharmacyId): Promise<boolean> {
    return this.store.remove(id);
  }

  async summary(facilityId?: string): Promise<PharmacySummary> {
  ...
  async analyzePharmacySignal1(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
  ...
  async analyzePharmacySignal2(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal3(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal4(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal5(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal6(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal7(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal8(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal9(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal10(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal11(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal12(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal13(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal14(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal15(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal16(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal17(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal18(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal19(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal20(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal21(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal22(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal23(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal24(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal25(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal26(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal27(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal28(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzePharmacySignal29(id: PharmacyId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
...
function matchesPharmacyFilter(item: PharmacyRecord, filter: PharmacyFilter): boolean {
