...
import type { BillingRecord } from '@/modules/billing/types/billing';
import { useBillingMutations } from '@/modules/billing/hooks/billing';

export function BillingDetailPanel({ id }: { id: string }) {
  const [record, setRecord] = useState<BillingRecord | null>(null);
  const [error, setError] = useState<string | null>(null);
  const { update, busy } = useBillingMutations();

  useEffect(() => {
    fetch(`/api/billing/${id}`)
      .then(async (r) => {
        if (!r.ok) throw new Error(await r.text());
        return r.json() as Promise<BillingRecord>;
      })
      .then(setRecord)
  ...
  return (
    <article className="cn-panel p-6 space-y-4">
      <header className="space-y-1">
        <p className="text-xs uppercase tracking-[0.2em] text-nest-500">Billing</p>
