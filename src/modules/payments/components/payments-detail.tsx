...
import type { PaymentsRecord } from '@/modules/payments/types/payments';
import { usePaymentsMutations } from '@/modules/payments/hooks/payments';

export function PaymentsDetailPanel({ id }: { id: string }) {
  const [record, setRecord] = useState<PaymentsRecord | null>(null);
  const [error, setError] = useState<string | null>(null);
  const { update, busy } = usePaymentsMutations();

  useEffect(() => {
    fetch(`/api/payments/${id}`)
      .then(async (r) => {
        if (!r.ok) throw new Error(await r.text());
        return r.json() as Promise<PaymentsRecord>;
      })
      .then(setRecord)
  ...
  return (
    <article className="cn-panel p-6 space-y-4">
      <header className="space-y-1">
        <p className="text-xs uppercase tracking-[0.2em] text-nest-500">Payments</p>
