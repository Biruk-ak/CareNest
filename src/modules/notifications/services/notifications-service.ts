...
import { notificationsCreateSchema, notificationsFilterSchema, notificationsUpdateSchema } from '../validators/notifications';
import { notificationsStore } from '../stores/notifications-store';
import { buildNotificationsId, nowIso, paginateNotifications } from '../utils/notifications';

/** Application service for Notifications domain operations */
export class NotificationsService {
  constructor(private readonly store = notificationsStore) {}

  async list(filter: NotificationsFilter = {}): Promise<NotificationsPage> {
    const parsed = notificationsFilterSchema.parse(filter);
    const all = this.store.getAll();
    const filtered = all.filter((item) => matchesNotificationsFilter(item, parsed));
    return paginateNotifications(filtered, parsed.page ?? 1, parsed.pageSize ?? 25, parsed.sortBy as keyof NotificationsRecord | undefined, parsed.sortDir);
  }

  async getById(id: NotificationsId): Promise<NotificationsRecord | null> {
    return this.store.getById(id) ?? null;
  }

  async create(input: NotificationsCreateInput, actorId: string, facilityId: string): Promise<NotificationsRecord> {
    const data = notificationsCreateSchema.parse(input);
    const stamp = nowIso();
    const record: NotificationsRecord = {
      id: buildNotificationsId(),
      title: data.title,
  ...
  async update(input: NotificationsUpdateInput, actorId: string): Promise<NotificationsRecord> {
    const data = notificationsUpdateSchema.parse(input);
    const existing = this.store.getById(data.id as NotificationsId);
    if (!existing) throw new Error('Notifications record not found');
    const next: NotificationsRecord = {
      ...existing,
      ...data,
      id: existing.id,
      updatedAt: nowIso(),
  ...
  async remove(id: NotificationsId): Promise<boolean> {
    return this.store.remove(id);
  }

  async summary(facilityId?: string): Promise<NotificationsSummary> {
  ...
  async analyzeNotificationsSignal1(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
  ...
  async analyzeNotificationsSignal2(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal3(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal4(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal5(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal6(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal7(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal8(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal9(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal10(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal11(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal12(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal13(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal14(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal15(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal16(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal17(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal18(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal19(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal20(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal21(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal22(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal23(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal24(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal25(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal26(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal27(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal28(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeNotificationsSignal29(id: NotificationsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
...
function matchesNotificationsFilter(item: NotificationsRecord, filter: NotificationsFilter): boolean {
