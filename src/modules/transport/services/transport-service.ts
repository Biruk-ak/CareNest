...
import { transportCreateSchema, transportFilterSchema, transportUpdateSchema } from '../validators/transport';
import { transportStore } from '../stores/transport-store';
import { buildTransportId, nowIso, paginateTransport } from '../utils/transport';

/** Application service for Transport domain operations */
export class TransportService {
  constructor(private readonly store = transportStore) {}

  async list(filter: TransportFilter = {}): Promise<TransportPage> {
    const parsed = transportFilterSchema.parse(filter);
    const all = this.store.getAll();
    const filtered = all.filter((item) => matchesTransportFilter(item, parsed));
    return paginateTransport(filtered, parsed.page ?? 1, parsed.pageSize ?? 25, parsed.sortBy as keyof TransportRecord | undefined, parsed.sortDir);
  }

  async getById(id: TransportId): Promise<TransportRecord | null> {
    return this.store.getById(id) ?? null;
  }

  async create(input: TransportCreateInput, actorId: string, facilityId: string): Promise<TransportRecord> {
    const data = transportCreateSchema.parse(input);
    const stamp = nowIso();
  ...
  async update(input: TransportUpdateInput, actorId: string): Promise<TransportRecord> {
    const data = transportUpdateSchema.parse(input);
    const existing = this.store.getById(data.id as TransportId);
  ...
  async remove(id: TransportId): Promise<boolean> {
    return this.store.remove(id);
  }

  async summary(facilityId?: string): Promise<TransportSummary> {
  ...
  async analyzeTransportSignal1(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
  ...
  async analyzeTransportSignal2(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal3(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal4(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal5(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal6(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal7(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal8(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal9(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal10(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal11(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal12(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal13(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal14(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal15(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal16(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal17(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal18(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal19(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal20(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal21(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal22(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal23(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal24(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal25(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal26(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal27(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal28(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeTransportSignal29(id: TransportId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
...
function matchesTransportFilter(item: TransportRecord, filter: TransportFilter): boolean {
