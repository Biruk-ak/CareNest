...
export function seedAdminRecords(count = 48): AdminRecord[] {
  const rows: AdminRecord[] = [];
  for (let i = 0; i < count; i++) {
    const id = `seed-admin-${String(i + 1).padStart(4, '0')}` as AdminId;
    const created = new Date(Date.UTC(2024, i % 12, (i % 27) + 1, 8 + (i % 10))).toISOString();
    rows.push({
      id,
      title: `Admin case #${i + 1}`,
      description: `Seeded admin workflow item #${i + 1} for CareNest facility operations.`,
      status: STATUSES[i % STATUSES.length],
      priority: PRIORITIES[i % PRIORITIES.length],
      visibility: VIS[i % VIS.length],
      residentId: `resident-${(i % 20) + 1}`,
      caregiverId: `caregiver-${(i % 12) + 1}`,
      tags: ['admin', `batch-${i % 5}`, i % 2 === 0 ? 'monitored' : 'routine'],
      notes: [`Initial note for admin #${i + 1}`, `Follow-up checkpoint ${(i % 3) + 1}`],
      metadata: { seedIndex: i, module: 'admin', weight: i * 1.5 },
      createdAt: created,
      updatedAt: created,
