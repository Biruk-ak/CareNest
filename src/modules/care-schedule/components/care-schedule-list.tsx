...
import { useCareScheduleList, useCareScheduleSummary } from '@/modules/care-schedule/hooks/care-schedule';
import { formatCareScheduleHeadline } from '@/modules/care-schedule/formatters/care-schedule';

export function CareScheduleListPanel() {
  const { items, total, loading, error, setFilter, reload } = useCareScheduleList({ page: 1, pageSize: 20 });
  const { summary } = useCareScheduleSummary();

  return (
    <section className="cn-panel p-6 space-y-5">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-nest-500">CareSchedule</p>
          <h2 className="font-display text-2xl text-nest-900">CareSchedule workspace</h2>
        </div>
      ...
      {loading && <p className="text-sm text-nest-600">Loading care-schedule records…</p>}
      ...
      <ul className="divide-y divide-nest-100">
        {items.map((item) => (
          <li key={item.id} className="py-3 flex items-start justify-between gap-4">
            <div>
              <Link href={`/app/care-schedule/${item.id}`} className="font-semibold text-nest-800 hover:text-nest-600">
                {formatCareScheduleHeadline(item)}
              </Link>
