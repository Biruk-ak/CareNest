...
import { housingCreateSchema, housingFilterSchema, housingUpdateSchema } from '../validators/housing';
import { housingStore } from '../stores/housing-store';
import { buildHousingId, nowIso, paginateHousing } from '../utils/housing';

/** Application service for Housing domain operations */
export class HousingService {
  constructor(private readonly store = housingStore) {}

  async list(filter: HousingFilter = {}): Promise<HousingPage> {
    const parsed = housingFilterSchema.parse(filter);
    const all = this.store.getAll();
    const filtered = all.filter((item) => matchesHousingFilter(item, parsed));
    return paginateHousing(filtered, parsed.page ?? 1, parsed.pageSize ?? 25, parsed.sortBy as keyof HousingRecord | undefined, parsed.sortDir);
  }

  async getById(id: HousingId): Promise<HousingRecord | null> {
    return this.store.getById(id) ?? null;
  }

  async create(input: HousingCreateInput, actorId: string, facilityId: string): Promise<HousingRecord> {
    const data = housingCreateSchema.parse(input);
    const stamp = nowIso();
    const record: HousingRecord = {
      id: buildHousingId(),
      title: data.title,
  ...
  async update(input: HousingUpdateInput, actorId: string): Promise<HousingRecord> {
    const data = housingUpdateSchema.parse(input);
    const existing = this.store.getById(data.id as HousingId);
    if (!existing) throw new Error('Housing record not found');
    const next: HousingRecord = {
      ...existing,
      ...data,
      id: existing.id,
      updatedAt: nowIso(),
  ...
  async remove(id: HousingId): Promise<boolean> {
    return this.store.remove(id);
  }

  async summary(facilityId?: string): Promise<HousingSummary> {
  ...
  async analyzeHousingSignal1(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
  ...
  async analyzeHousingSignal2(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal3(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal4(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal5(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal6(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal7(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal8(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal9(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal10(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal11(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal12(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal13(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal14(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal15(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal16(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal17(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal18(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal19(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal20(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal21(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal22(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal23(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal24(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal25(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal26(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal27(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal28(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeHousingSignal29(id: HousingId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
...
function matchesHousingFilter(item: HousingRecord, filter: HousingFilter): boolean {
