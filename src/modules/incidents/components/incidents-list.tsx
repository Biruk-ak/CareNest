...
import { useIncidentsList, useIncidentsSummary } from '@/modules/incidents/hooks/incidents';
import { formatIncidentsHeadline } from '@/modules/incidents/formatters/incidents';

export function IncidentsListPanel() {
  const { items, total, loading, error, setFilter, reload } = useIncidentsList({ page: 1, pageSize: 20 });
  const { summary } = useIncidentsSummary();

  return (
    <section className="cn-panel p-6 space-y-5">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-nest-500">Incidents</p>
          <h2 className="font-display text-2xl text-nest-900">Incidents workspace</h2>
        </div>
      ...
      {loading && <p className="text-sm text-nest-600">Loading incidents records…</p>}
      ...
      <ul className="divide-y divide-nest-100">
        {items.map((item) => (
          <li key={item.id} className="py-3 flex items-start justify-between gap-4">
            <div>
              <Link href={`/app/incidents/${item.id}`} className="font-semibold text-nest-800 hover:text-nest-600">
                {formatIncidentsHeadline(item)}
              </Link>
