...
import { incidentsCreateSchema, incidentsFilterSchema, incidentsUpdateSchema } from '../validators/incidents';
import { incidentsStore } from '../stores/incidents-store';
import { buildIncidentsId, nowIso, paginateIncidents } from '../utils/incidents';

/** Application service for Incidents domain operations */
export class IncidentsService {
  constructor(private readonly store = incidentsStore) {}

  async list(filter: IncidentsFilter = {}): Promise<IncidentsPage> {
    const parsed = incidentsFilterSchema.parse(filter);
    const all = this.store.getAll();
    const filtered = all.filter((item) => matchesIncidentsFilter(item, parsed));
    return paginateIncidents(filtered, parsed.page ?? 1, parsed.pageSize ?? 25, parsed.sortBy as keyof IncidentsRecord | undefined, parsed.sortDir);
  }

  async getById(id: IncidentsId): Promise<IncidentsRecord | null> {
    return this.store.getById(id) ?? null;
  }

  async create(input: IncidentsCreateInput, actorId: string, facilityId: string): Promise<IncidentsRecord> {
    const data = incidentsCreateSchema.parse(input);
    const stamp = nowIso();
    const record: IncidentsRecord = {
      id: buildIncidentsId(),
      title: data.title,
  ...
  async update(input: IncidentsUpdateInput, actorId: string): Promise<IncidentsRecord> {
    const data = incidentsUpdateSchema.parse(input);
    const existing = this.store.getById(data.id as IncidentsId);
    if (!existing) throw new Error('Incidents record not found');
    const next: IncidentsRecord = {
      ...existing,
      ...data,
      id: existing.id,
      updatedAt: nowIso(),
  ...
  async remove(id: IncidentsId): Promise<boolean> {
    return this.store.remove(id);
  }

  async summary(facilityId?: string): Promise<IncidentsSummary> {
  ...
  async analyzeIncidentsSignal1(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
  ...
  async analyzeIncidentsSignal2(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal3(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal4(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal5(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal6(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal7(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal8(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal9(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal10(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal11(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal12(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal13(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal14(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal15(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal16(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal17(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal18(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal19(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal20(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal21(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal22(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal23(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal24(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal25(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal26(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal27(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal28(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeIncidentsSignal29(id: IncidentsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
...
function matchesIncidentsFilter(item: IncidentsRecord, filter: IncidentsFilter): boolean {
