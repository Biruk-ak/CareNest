...
import type { ReportsRecord } from '@/modules/reports/types/reports';
import { useReportsMutations } from '@/modules/reports/hooks/reports';

export function ReportsDetailPanel({ id }: { id: string }) {
  const [record, setRecord] = useState<ReportsRecord | null>(null);
  const [error, setError] = useState<string | null>(null);
  const { update, busy } = useReportsMutations();

  useEffect(() => {
    fetch(`/api/reports/${id}`)
      .then(async (r) => {
        if (!r.ok) throw new Error(await r.text());
        return r.json() as Promise<ReportsRecord>;
      })
      .then(setRecord)
  ...
  return (
    <article className="cn-panel p-6 space-y-4">
      <header className="space-y-1">
        <p className="text-xs uppercase tracking-[0.2em] text-nest-500">Reports</p>
