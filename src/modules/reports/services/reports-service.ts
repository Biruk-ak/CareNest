...
import { reportsCreateSchema, reportsFilterSchema, reportsUpdateSchema } from '../validators/reports';
import { reportsStore } from '../stores/reports-store';
import { buildReportsId, nowIso, paginateReports } from '../utils/reports';

/** Application service for Reports domain operations */
export class ReportsService {
  constructor(private readonly store = reportsStore) {}

  async list(filter: ReportsFilter = {}): Promise<ReportsPage> {
    const parsed = reportsFilterSchema.parse(filter);
    const all = this.store.getAll();
    const filtered = all.filter((item) => matchesReportsFilter(item, parsed));
    return paginateReports(filtered, parsed.page ?? 1, parsed.pageSize ?? 25, parsed.sortBy as keyof ReportsRecord | undefined, parsed.sortDir);
  }

  async getById(id: ReportsId): Promise<ReportsRecord | null> {
    return this.store.getById(id) ?? null;
  }

  async create(input: ReportsCreateInput, actorId: string, facilityId: string): Promise<ReportsRecord> {
    const data = reportsCreateSchema.parse(input);
    const stamp = nowIso();
    const record: ReportsRecord = {
      id: buildReportsId(),
      title: data.title,
  ...
  async update(input: ReportsUpdateInput, actorId: string): Promise<ReportsRecord> {
    const data = reportsUpdateSchema.parse(input);
    const existing = this.store.getById(data.id as ReportsId);
    if (!existing) throw new Error('Reports record not found');
    const next: ReportsRecord = {
      ...existing,
      ...data,
      id: existing.id,
      updatedAt: nowIso(),
  ...
  async remove(id: ReportsId): Promise<boolean> {
    return this.store.remove(id);
  }

  async summary(facilityId?: string): Promise<ReportsSummary> {
  ...
  async analyzeReportsSignal1(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
  ...
  async analyzeReportsSignal2(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal3(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal4(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal5(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal6(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal7(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal8(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal9(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal10(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal11(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal12(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal13(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal14(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal15(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal16(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal17(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal18(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal19(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal20(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal21(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal22(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal23(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal24(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal25(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal26(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal27(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal28(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
  ...
  async analyzeReportsSignal29(id: ReportsId): Promise<{ score: number; label: string; details: string[] }> {
    const record = await this.getById(id);
    if (!record) return { score: 0, label: 'missing', details: ['Record not found'] };
...
function matchesReportsFilter(item: ReportsRecord, filter: ReportsFilter): boolean {
